...
The key lives in `plugins.updater.pubkey` in `tauri.conf.json` and is compiled into every build. Its private half exists only as a GitHub secret (see [Signing](build.md#signing)). `tauri-plugin-updater` verifies every downloaded bundle against it before touching the installed app. Were the key editable alongside the URL, the variable source would stop being a convenience and become a way to install arbitrary code — which is why `update.json` has no field for it.

**A signature alone is not enough, and this is the part that is easy to get wrong.** The manifest is not signed; only the artifact it points at is. Without further work a hijacked manifest can therefore pair a high version number with the URL *and genuine signature* of an older release — every check passes, and the user is downgraded onto a version whose weaknesses are already public. `requireSignedVersion: true` closes that: since `@tauri-apps/cli` 2.11.5 the signing step records the app version inside the signature's trusted comment, which minisign covers, and the plugin refuses any response whose announced version disagrees. The flag and the CLI floor belong together — the flag against older signatures rejects *every* update. With both in place the earlier claim holds: the worst a wrong, hijacked or mistyped source can do is deliver nothing, or deliver something that does not verify.
...
`install_update` installs only what the last `check_for_update` found and showed the user — the result is held in app state rather than re-fetched, so what gets installed is what was displayed.

It also refuses on an install method that cannot be replaced, and on Linux that test is deliberately stricter than it looks. `APPIMAGE` is an ordinary environment variable and is **inherited by child processes**: a `.deb`-installed Emerald started from a terminal or editor that is itself an AppImage sees that foreign path, and the plugin takes exactly that value as the install target. Checking the variable alone would therefore, in the worst case, overwrite an unrelated application. `install_supported()` additionally requires the running executable to sit under `{temp}/.mount_…`, which is where an AppImage actually runs from — the same probe `tauri-utils` uses, though it only warns. A hand-extracted AppImage falls to "not replaceable", which is correct: there is no single file to swap.
...
Note on [Vault Layout → Vault Directories as a Trust Boundary](architecture.md#vault-layout) above: `resolve_allowed_roots` already includes `document_dir()`, so a vault at its default location — `{documentDir}/Emerald Vaults/{name}` — sits *inside* the allowed roots, and so does the `backup/` folder inside it that the database export offers as its default destination. For a vault *outside* the roots, `ensure_backup_dir` refuses up front — offering a default there would only have `write_file` refuse the write a moment later — and the export dialog falls back to a plain filename, and reading or writing a document into it (a backup file, a Markdown export) works normally. The "out of reach" case is a vault deliberately relocated to a folder the fixed roots don't cover — another drive, a folder outside `~` entirely — not the default location, and not the old `{appDataDir}/vaults/{id}` location either, since `app_data_dir()` is also one of the roots.

### The registry can arrive pre-populated, from a directory this installation does not manage

The premise above — a path only becomes trusted once it is in the registry, and the registry is filled exclusively by `register_vaults`, an ordinary frontend command — still holds without exception: no other code path writes into `VaultRegistry`. What changed with the 0.2.x identifier rename is where the *file* `register_vaults`'s entries ultimately came from. On a first start, `vaults.json` can be a copy `adopt_previous_identifier_dirs` (`src-tauri/src/vault.rs`, see [`architecture.md`](architecture.md#adopting-a-previous-identifiers-data)) placed there from the previous identifier's directory — one this installation does not itself write to or otherwise control the contents of.

This does not widen what a vault path can be: `vaults.json` has always held arbitrary absolute paths the user chose in a folder dialog, on either side of the copy. What the adoption adds is a rewrite step, `retarget_registry`, that has to run on a file this code did not itself produce: it rewrites a stored path from the old `appDataDir` root onto the new one, for any vault that lived under the pre-0.2.1 layout (`{appDataDir}/vaults/{id}`). `strip_prefix`/`join` are purely lexical, so a `..` surviving inside a stored path could otherwise walk the rewritten result back out of the new data directory. `retarget_registry` refuses that case: an entry is only rewritten when everything past the stripped prefix is an ordinary path component (no `..`, no root/prefix component); anything else is left exactly as it was rather than being retargeted somewhere unintended. A vault left unrewritten this way keeps pointing exactly where it pointed before, which — because the adoption copies rather than moves — still exists: the previous installation's directory. It therefore still opens, from there, and the user's writing goes to the old folder while the adopted copy beside it grows stale. That is a worse outcome than a rewrite would have been in the ordinary case, and a better one than a rewrite would have been in this one, which is the whole point of the rule: not retargeting a path is recoverable, retargeting it somewhere unintended is not. It becomes a plain "Folder not found" only once the old directory is deleted.
...
**Thumbnail size cap.** The 512 KB limit lives in one place now — `THUMBNAIL_MAX_BYTES` in `src/lib/thumbnail.ts` — imported by the three capture sites in `AltarView` and enforced inside `canvasToCappedThumbnail`, the shared WebP/JPEG/PNG quality ladder used by the altar thumbnails. This bounds the size of data stored in `altars.thumbnail_data` (`operations.thumbnail_data` is a legacy column, empty since v41) and prevents oversized blobs from accumulating in the SQLite file.
...
**Emerald import (`src/lib/emeraldFormat.ts`).** After image paths are remapped, the content string is passed through `DOMPurify.sanitize`. The sanitisation configuration preserves TipTap-specific attributes (`data-type`, `data-id`, `data-entry-type`, `data-label`, `data-icon`, `data-align`) and strips everything else that DOMPurify would remove by default (script tags, event handler attributes, etc.). `data-align` (image alignment) is intentionally not added to the PDF export sanitisation allowlist — that path never reads the attribute, and an image's alignment survives PDF export through its inline `margin-left`/`margin-right` instead.
...
**Content blocks (`src/lib/blocks/`, `src/components/blocks/`).** Entry content is now a stack of `<section data-block>` wrappers, and imported content reaches the block parser before TipTap's schema ever sees it — so the block layer treats every wrapper as untrusted:

- **Parser.** `parseBlocks` keeps only `data-*` attributes of a section (no `onclick`, `class`, `style`), decodes entities, and `blockSectionHtml` writes every attribute back double-quoted through `escapeHtml`, so a value cannot break out of its quotes. Editor HTML is passed through `neutralizeSectionTags` before storing, so a `<section` inside an attribute value (older WebKit leaves `<` unescaped there) cannot split an entry.
- **Unknown block types.** A block the app cannot render shows its inner HTML via `UnknownBlock`, the one `dangerouslySetInnerHTML` in the block layer. It runs DOMPurify with a **tight allowlist** — text structure only (`p`, headings, lists, `a`, `img`, `dl`, tables …) and only `href`/`src`/`alt`/`title` — because DOMPurify's default profile keeps `style`, `class`, `<style>` and forms: imported content could otherwise cover the window with a fake UI (`position: fixed`, the app's own Tailwind classes) or post a form to a remote server. `.block-unknown` additionally sets `contain: paint`, and clicks on links inside it are intercepted and opened with `openUrl` for http(s) only, never navigating the app window.
- **Fields block.** Values are rendered as React text; the readable fallback is written through `escapeHtml`. Element ids come from content: they must match `[A-Za-z0-9_-]{1,64}` and must not name an `Object.prototype` property, and the value/slot records have no prototype — a crafted `constructor`/`__proto__` id used to read a built-in as slot HTML and throw during render. Image slots resolve through `imageSrc`, which confines stored names to the vault's image folder.
- **User-built blocks.** `block_definitions` rows arrive from `.emeralddb` backups (`INSERT OR IGNORE` through the column-whitelisting `insertRows`) and `.emerald` files (`meta.blockDefinitions`, ids restricted to `[\w-]{1,100}`). Either way their `elements`/`display` JSON is read through `parseDefinitionElements`/`parseDefinitionDisplay` — the same element validation as a copy in content (safe ids, known kinds, duplicates dropped) — and name and emoji are rendered as React text. An import never overwrites an existing definition, and it never rewrites entries: copies render from their own content.
- **Failure isolation.** Every block renders inside `BlockErrorBoundary`: a block that throws falls back to `UnknownBlock` instead of blanking the whole app (which, with the open tab restored at startup, would otherwise repeat on every launch).

**Templates.** A template's `content` is a block stack in the same stored format as an entry's, so inserting one (`instantiateTemplateBlocks`) is a block-tree operation, not a fresh HTML parse — it goes through the same block-layer defenses above, not TipTap's Markdown path. The one place raw text still becomes HTML is the one-time routine→template conversion (migration v44 and the `.emeralddb` import path that runs the same conversion, `src/lib/migrateRoutinesToTemplates.ts`): a routine's plain-text/Markdown `content` is rendered with a dedicated `Marked` instance whose `html`/`link`/`image` renderers escape raw HTML outright and drop any link/image whose URL isn't `http(s):`/`mailto:` (rendering the visible text only) — stricter than the routine-drop code path it replaces, which relied on `marked`'s default HTML output plus TipTap's schema to keep the result safe.
...
The main window has an **explicit, hand-written CSP** in `tauri.conf.json` (`app.security.csp`), not Tauri's default. `script-src` is `'self'`, but that does not block *every* inline script: Tauri hashes each inline `<script>` element in the built HTML at compile time and appends its sha256 to `script-src` (`tauri-codegen`'s `inject_script_hashes`, consumed by `manager/mod.rs`'s `set_csp`), so a specific, unmodified inline script — like the boot script at the top of `index.html`'s `<head>` that sets the theme and arms the loading screen's hard fallback timer before the bundle exists, see [Loading Screen and Boot Order](architecture.md#loading-screen-and-boot-order) — still runs under `'self'`; any other or modified inline script is still blocked. `<style>` elements are treated differently: Tauri gives each one a nonce, and a nonce present in `style-src` makes CSP3 ignore a neighbouring `'unsafe-inline'` — which would silently block every `style={{…}}` attribute in the app (the sidebar widths in `AppShell`, among others) in a packaged build. This is why the loading screen's CSS lives in `public/splash.css`, loaded via a `<link>`, rather than a `<style>` block in `index.html`. `img-src` additionally allows `emerald-img:` and `http://emerald-img.localhost` — the same scheme in the two forms the platforms serve it as (WebView2 maps custom schemes onto `http`, the other engines do not). Three remote origins are allowed for typography: `style-src`/`connect-src` include `https://fonts.googleapis.com` and `connect-src`/`font-src` include `https://fonts.gstatic.com`. That is the app's one network dependency: `src/main.tsx` injects the Google Fonts stylesheet link at runtime (with `preconnect` hints in `index.html`), and the app falls back to system font stacks when offline — nothing else may leave the machine. `form-action 'none'` forbids form submissions outright: the app has no HTML form that posts anywhere, and `default-src` does not cover form targets, so without it a form that slipped into rendered content could send what the user types to a remote server. The export HTML for PDF export is written to a unique file in the OS temp directory and loaded by a hidden webview over a `file://` URL; that document carries **no CSP at all**, not the main window's — a `file://` document has nothing to inherit it from. What controls that path instead is `htmlEscape` and `DOMPurify.sanitize` in `src/lib/export.ts` (see [HTML Escaping in Exports](#html-escaping-in-exports) and [HTML Sanitisation](#html-sanitisation) above), and the fact that the export file is removed from disk as soon as the export finishes. The frontend still pre-renders the internal-link chip transformation in TypeScript (`transformInternalLinks` in `src/lib/export.ts`) before handing the HTML to the backend, but that is an implementation convenience, not a CSP-driven necessity.
