...
| `sidebar` | Left sidebar rail strings: `back` / `forward` (navigation-history button titles), `collapseList` / `expandList` (entry-list toggle button title), `collapseProperties` / `expandProperties` (right-sidebar toggle button title, also on the rail) |
