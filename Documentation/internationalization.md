...
| `listView` | View/sort/grouping mode labels for the list toolbar, including `cardsWide` (the full-width card layout), `grouping`/`ungrouped` (the grouping axis, independent of sort), `type` (Trash's own word for what it groups by), and `countDesc` ("Most used first" — the usage-count sort offered only by Tags) |
...
| `sidebar` | Left sidebar rail strings: `allEmpty` (empty-state message for the entry-list's "All" tab). The rail itself carries no toggle buttons — `collapseList`/`expandList`/`collapseProperties`/`expandProperties` were removed along with them; all three sidebars toggle only through `menu.rail`/`menu.entryList`/`menu.properties` now |
...
| `blocks` | Content-blocks UI shared by Journal/Wiki/Operations (block frame actions, the sidebar block manager, unknown-block fallback) plus the Blocks view. `types.*` names the block types themselves (`text`, `fields`, `sigilCalc`, `sigilCanvas`, `sigilCharge`); `kinds.*` names a field-block element kind, one entry per `ElementKind` (`text`, `shorttext`…`moon`, plus `altar` and the three sigil-part kinds `sigilCalc`/`sigilCanvas`/`sigilCharge` — shared between the "Add block" preset menu and the Blocks builder's element picker); `fields.*` is the field block's own strings (inputs, `hideEmpty`/`readOnly` display rules); `sigil.*` covers the standalone sigil blocks and, since a part behaves identically, their in-block counterparts too (`covers`/`noTools` for what a charge covers, `noRevealDate`/`hiddenUntilUnloaded`/`loadConfirmNoDate` for a charge loaded without a target date); `altar.*` is the altar field kind (`search`, `missing`, `open`); `library.*` is the Blocks builder (`prefill`/`prefillHint` for the generic "Prefill" toggle, `calcMode`/`calcModes.*`/`brushPreset`/`brushPresetHint`/`chargePrefillHint`/`chargeCoversAll`/`chargeNoTools` for a sigil part's own settings, `skippedLocked` alongside `skippedEditing` in the update/remove-all-copies notice — since templates can hold copies too, `inTemplates`/`outdatedTemplates` extend the usage count and `skippedDrafts` extends the same notice for a template whose page is open or has an unsaved draft, and `updateAllConfirmCopies`/`deleteFromCopies*`/`removeAndDeleteCopies*` are entry-or-template phrasings of the confirmation/warning text the plain `deleteFromEntries*`/`removeAndDelete*` keys used before) |
...
| `templates` | The templates dashboard and template page: `builtin.sigil` (the display name for the built-in `core-sigil` template, resolved the same way `categories.builtin.*` names a built-in category), `entryTitle`/`content`/`assignments` (the page's own fields), `overview.*` (the dashboard's "Defaults" section), `saveAsTemplate` (the entry context-menu action), `insert.*` (the editor's picker, apply dialog, empty-entry suggestions, and the "applied" notice with its Undo/"Other template" actions). Replaces the former `routines` block (`newRoutine`/`namePlaceholder`/`contentPlaceholder`) now that routines are templates |
| `tags` | Tags view strings, including `new`/`name` (the create/rename row), `nameTaken`, `color` (the colour-dot picker), and `unused`/`unusedHint` |
| `properties` | Right sidebar Properties panel strings |
| `filters` | Filter panel strings, including `module` (the Tags view's module-filter chip group label) |
...
| `trash` | Trash view strings, including `categories`/`blockDefinitions`/`templates` (the trash's own section labels for those three kinds). Its former `sure`/`confirmYes`/`confirmNo`/`loading` keys moved to `common.*` (see above), since Tags and the Altar library shared the exact same three-string confirmation under this module's name |
