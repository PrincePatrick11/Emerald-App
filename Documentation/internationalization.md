...
| `categories` | The one category block for Wiki/Operations/Tasks/Altar since v38: `add` (the "+ Category" dialog title), `name` (name-input placeholder), `uncategorized`, `nameTaken` (duplicate-name error), `builtin.other` / `builtin.sigils` (the two true built-ins' display names), `starter.*` (the eight starter-category names, used once to name them at fresh-vault creation — see [Built-in Category Names](#built-in-category-names)). Replaces what used to be four near-identical `addCategory`/`categoryName`/`uncategorized` triples scattered across `wiki`, `operations`, `altar`, and `tasks` |
| `listView` | View/sort mode labels for list toolbar |
| `undo` | Undo toast messages |
| `nav` | Sidebar navigation labels, including `home` (the rail's Home button, above Journal), `all` (the entry-list's "All" tab), and `vaults` (the rail button that opens `VaultModal`) |
| `sidebar` | Left sidebar rail strings: `collapseList` / `expandList` (entry-list toggle button title), `collapseProperties` / `expandProperties` (right-sidebar toggle button title, also on the rail), `allEmpty` (empty-state message for the entry-list's "All" tab) |
