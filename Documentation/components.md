...
| `CategorySelect` | themed category-assignment picker built on `Dropdown`, replacing two native `<select>`s (Wiki/Operations properties panels) and `TaskRow`'s hand-rolled menu; also used by the altar library's `ItemModal` (add/edit item dialog), replacing a row of category chips | `variant` (`field` for properties panels and `ItemModal` — portals; `chip` for `TaskRow`), `align`, `title`, `placeholder` |
