...
| `DashboardItem` | `children` | everything inside the clickable frame — icon, title, meta, a sigil preview, an altar canvas |
...
| `DashboardItem` | a clickable item inside a `Dashboard`: the `panel-interactive` frame (`layout: 'row'` or `'card'`), click opens `view` via `setActiveView`, middle-click opens it via `openViewInNewTab`. With `editing` it renders the same frame as a plain `div` without either handler, for in-place renaming. Used by `renderItem` in Journal, Wiki, Operations and Tags and by `AltarCard`/`AltarListRow` — the hand-written copies had drifted (Journal and Altar had no middle-click). `HomeView`'s sections still hand-write the same button with denser padding | `view`, `layout`, `onContextMenu`, `editing`, `children` |
| `InlineConfirm` | the "Sure? Yes, delete / Cancel" confirmation in place of a row's actions — `danger`/`neutral` tone buttons. When it shows stays the caller's (a `confirmingId` state the first click sets). Used by Categories, Tags, Trash (rows, cards, bulk delete, empty trash) and `AltarItemModal` — Trash had drifted to `variant` buttons with hand-tuned padding, Categories had no question | `message` (default "Sure?"), `confirmLabel`, `small` (24px buttons for dense headers), `variant: 'banner'` (a full-width red strip, for dialogs) |
