...
| `lib/groupBy.ts` | `groupBy(items, keyFn)` → `DashboardGroup[]`, and `groupByMonth` (keyed by the localized month via `formatMonthGroup`). Replaces five month-grouping loops plus two category remaps and two Trash category maps |
