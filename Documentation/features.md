...
**Blocks.** An entry's body is a stack of blocks — so far text blocks, each with the full editor above. In edit mode every block shows a subtle frame: a grip on the left to drag it up or down, its type, and a "…" menu to duplicate or remove it. Hovering between two blocks reveals a "+" that inserts a new one there; the "+ Add block" line below the last block is always visible. Read mode shows none of this — the blocks flow like one document. There is one formatting toolbar, pinned to the top while scrolling, and it acts on whichever text block you are typing in; a link added from the "Linked entries" field, an image dropped from the file explorer, or an entry dragged in from the left sidebar's list lands in the text block you last worked in (drags: the one under the pointer). Cancel reverts block changes along with the text. The same applies to Wiki articles and Operations, sigils included.

**Field blocks.** Besides text, "Add block" offers ten kinds of field: short text, number, date, choice, yes/no, checklist, link (to any entry), image, moon phase, and altar — a link restricted to altars, shown full width with the altar's own picture; clicking it opens the altar. In read mode a field shows as a small label with its value, set in the same typeface as the text around it; an empty field is left out. While editing, each field has its own input — a date picker, a dropdown for a choice or a moon phase, a switch for yes/no, a list with "Add item" for a checklist, a search for a link, "Choose image" for an image. The block's section in the sidebar holds its settings: the field's label, a choice's options (added, renamed and removed there), whether the field is hidden in read mode while empty, and "Fully read-only". Checklist items and yes/no switches can be toggled straight from read mode, without entering edit mode — unless the block is set to fully read-only. A field's value is part of the entry: search finds it, links in a link field count as the entry's links (and show up in "Linked entries"), and images in an image field are kept by the unused-image cleanup.

**Managing blocks in the sidebar.** While editing, a collapsible "Blocks" section below the Properties panel lists the entry's blocks: drag a row by its grip to reorder, use the eye to hide a block, and the "…" menu to rename it, to show its title as a small heading in read mode, to duplicate or to remove it; "+ Add block" appends a new one. Clicking a row selects it (and jumps to the block); if the block has settings of its own — a field's label, a choice's options, hide-when-empty, fully read-only — they open in a box right under the row, like the Altar's placed elements. Reordering has no animation: the dragged row simply jumps into place. A hidden block stays in the entry with everything in it — greyed out and marked while editing, simply absent in read mode. A text block with no real content (only empty paragraphs or line breaks) disappears from read mode the same way, without being marked hidden — it keeps showing its blank space while editing, so it can still be found and written into. Renaming changes only what the block is called (in its frame, the list, and — if switched on — as its read-mode heading); an empty name goes back to the type's name. In read mode, an entry with more than one block keeps the list as an outline: clicking a row scrolls to that block and briefly highlights it; hidden blocks are shown there dimmed with an eye-off mark. All of this is part of the entry's text, so Cancel reverts it too.

**Your own blocks.** The **Blocks** view (rail, above Tags) lists the blocks you've built, on the same dashboard every other module uses — icon, name, field and entry count, an "outdated" dot, an "Unsaved" marker for a draft left open elsewhere; the header (title, count, "New block", search, and view — list, cards, wide cards, or a timeline by month of last change — and sort — last changed, name, most used — both applying to your own blocks only) lives in the right sidebar. The main list itself opens with a collapsible "Your blocks" divider over your own blocks, and below them — laid out like the Altar library under the altars — a collapsible "Built-in blocks" section lists the built-in blocks too: the same ones "Add block" offers, text, every field kind and the three sigil blocks, as a plain, non-editable list (icon and label in columns, no frames, not clickable). A description tooltip still shows on text and the sigil blocks. The dashboard's search filters both sections. Clicking one of your own blocks opens it as its own page: a breadcrumb back to the list, icon and name as the title, and its fields below — add by kind (including **Text**, the same rich text editor as the Text block), label, reorder them (drag by the grip), remove. Its icon (an emoji or an image), the display rules — hide empty fields in read mode, show the block's title, fully read-only — and its usage sit in the right sidebar next to Done, Delete and Cancel; without the sidebar open, Done and Cancel sit in the page's own top bar instead. No code is needed to assemble one — a "Ritual log" with date, choice and checklist, for example — and a block left unfinished keeps its draft if you switch to another module and back, marked "Unsaved" in the list until you return and press Done. The block then appears under "Add block" in every entry, after the built-in ones. What lands in an entry is a **copy**: changing the block later leaves existing entries as they are. While editing such an entry, the block's frame shows "Newer version", and its "…" menu offers "Update block" (Cancel reverts it like any other change); the block's own page shows how many entries use it and how many of them carry an older version, and "Update all" brings them up to date at once — except an entry that is open for editing right then, or an entry where a loaded sigil holds the copy (see below), which is reported separately and left alone. Updating keeps every value; new fields arrive empty (or with their prefill, see below), and a field you removed keeps its data in the entries but stays hidden — the page lists removed fields and can bring them back. Deleting a block moves it to the trash and leaves every copy in the entries untouched; to remove it from all entries as well, tick "Also remove it from N entries" and confirm a second time — that part can only be undone from a backup. An empty list of your own blocks shows a hint to build your first one; the built-in section stays below it regardless.

Every field kind can be **prefilled**: tick "Prefill" on a field in the builder and give it a starting value — a checklist with its points already there, yes/no already set to "Yes", a link or altar already pointing at a chosen entry, an image already chosen — and every new copy of the block starts with it; existing copies are unaffected, and a copy created before the prefill was added keeps its own value when the block updates. A block can also hold the three **sigil parts** — calculator, drawing and charge — which work exactly like the standalone sigil blocks below, only living inside a block you built alongside its other fields. The builder lets a calculator part choose how it reaches its letter bank (automatic, manual, or both — the default), a drawing part preset the starting brush colour and size, and a charge part preset its lock and which of the block's own calculators and drawings it covers. A copy that holds a loaded sigil, whether as a full block or as one of these parts, is never touched by "Update block" or "Update all", and never removed by "Also remove it from entries" — it is reported as skipped instead, so it keeps hiding what it charged; duplicating a block a charge has hidden or locked isn't offered, since the copy wouldn't be covered by that charge.
...
**Changing type.** In edit mode, a "Type" row — at the top of the Properties panel, above Category — shows Journal, Operations, and Wiki as an icon toggle. Picking a different one moves the entry into that module right there: it keeps its id, title, content, tags, and creation date, and everything pointing at it follows — link chips elsewhere (including in templates and in Trash), the entry's own open tab and history, task links, and the link default of any custom block that pointed at it. Wiki and Operation keep category, icon, and cover image when switching between each other; switching to Journal drops them, asking first if any are set, since the Journal has none of the three. Whatever template is the default for the new combination is applied the same way a category change applies one, offering the same Undo / "Other template" notice. Tasks and Altar items have a different data model and take no part in this.
...
**List views.** The journal list supports four layouts (List, Cards, Cards in full width, Timeline), four sort orders (newest first, oldest first, A→Z, Z→A), and a separate Grouping toggle — grouped or ungrouped — which for the journal groups by **moon phase**. Grouped, the list renders one collapsible group per phase — chevron, moon emoji, phase name, entry count — in fixed lunar-cycle order; a phase with nothing in it is left out, the same way an empty category group is elsewhere. Entries without a recognized moon phase — none at all, or one an older import wrote that isn't part of the cycle — get their own "Without moon phase" group. Each group can be folded independently, matching the category groups in Wiki, Operations and Tasks. The filter panel's moon-phase chips are a different list from the groups above: all eight phases always show there regardless of use, since the lunar cycle itself is fixed rather than growing like a category list, plus an "All" chip that clears the selection; a "Without moon phase" chip is offered whenever such entries exist, or for as long as it's selected. Deselecting every phase hides all entries. Search filters by title *or* tag name. **Timeline view locks out A→Z/Z→A sorting and the Grouping toggle** — they're shown greyed out with a tooltip explaining why, since the timeline already orders entries by date and groups by month on its own; the same restriction applies wherever else Timeline is offered as a layout (Wiki, Operations, the Altar dashboard).
...
**Categories.** An article can belong to one category — the same category list Operations, Tasks, and Altar items share (see Altar's "Item categories" below for how the shared list works). A new article starts with none and shows under "Uncategorized" until one is chosen in the properties panel. A fresh vault starts with eight ordinary, renameable/deletable categories (Paradigm, Ritual, Meditation, Herbs, Crystals, Candles, Deities, Tools) and no built-in ones beyond **Sigils**. Categories are created, renamed and deleted in the Categories view (see below), not here. The Grouping toggle groups articles by category, independent of the sort order; each category's group can be collapsed and expanded, and its header has its own "+" button that creates a new article directly in that category. A category group with nothing in it is left out. Moving a category to Trash leaves its articles' category association as-is — they show in the "Uncategorized" group at the end of the list until the category is restored (bringing them back) or deleted permanently, which then leaves them without one for good. The category filter (in the toolbar's filter panel) lists only the categories actually in use by wiki articles rather than the full global list, so categories only used by Operations, Tasks or Altar don't clutter it here; an "All" chip clears the selection, and an "Uncategorized" chip is offered whenever an article has no category, staying visible for as long as it's selected even once the last such article is gone.
...
**Categories.** The same shared category list as Wiki, Tasks, and Altar items — see below. One category is a true built-in: **Sigils** (see Sigil Workflow); everything else can be renamed or deleted freely, including categories once called "Servitors" and "Other". A new operation starts without a category. Categories are created, renamed and deleted in the Categories view (see below). Categories have a name and an emoji; the Grouping toggle groups operations by category independent of the sort order, and their group in the list can be collapsed and expanded — each category's header also has its own "+" button that creates a new operation directly in it. A category group with nothing in it is left out. Moving a category to Trash leaves its operations' category association as-is — they show in the "Uncategorized" group at the end of the list until the category is restored (bringing them back) or deleted permanently, which then leaves them without one. The category filter lists only the categories actually in use by operations rather than the full global list; an "All" chip clears the selection, and an "Uncategorized" chip is offered whenever an operation has no category, staying visible for as long as it's selected.

**Status, end date and version are a block.** They used to be fixed properties — an active/inactive badge under the title, a status filter and a dot in the list, end date and version in the sidebar. Now they are the block **Status** (Active — yes/no, End date, Version), a user-built block like any other: add it to an operation via "Add block", toggle "Active" straight from read mode, and rename, extend or delete it in the Blocks view. Operations that had any of the three set got the block at the top of their text automatically when the vault was first opened with this version (the block itself is only created if some operation needed it); older backups, `.emerald` and Markdown files are converted the same way on import. Filtering the list by block values is planned.
...
A sigil is an operation with three blocks — **Sigil calculator**, **Sigil drawing** and **Charge** — alongside text and any other block. A new operation in the **Sigils** category starts with all three; in any other operation they can be added via "Add block".
...
3. In the charge block, optionally set a reveal date, link a charging technique — any wiki article — choose what charging locks (the whole entry, as before, or only the calculators and drawings it covers), and, when an entry holds more than one calculator or drawing, which of them this charge actually covers (all of them by default).
4. In read mode, charge the sigil — optionally after a timer — and confirm. A charge without a reveal date hides what it covers until you unload it, rather than showing an unset date. Until the reveal date (or the unload), the calculators and drawings it covers are hidden (also in search) and left out of exports; from the reveal date on they show again, still locked. "Unload sigil" (with confirmation) lifts the lock. A duplicated sigil comes unloaded, and a block a charge is currently hiding or locking can't be duplicated at all — the copy would carry the same charge without actually being covered by it.
...
Servitors use the standard operations editor with no special fields beyond the common operation fields. There is no longer a dedicated built-in category for them — file a servitor under whichever category fits, including one you name "Servitors" yourself.

## Altar

The altar is a virtual arrangement of symbolic objects on a canvas.

**Dashboard.** Opening the Altar section shows a dashboard listing all your altars with search, sort, and view-mode options (List / Cards / Cards in full width / Timeline), under its own collapsible "Altars" heading. Click any altar to open it; use the breadcrumb back button or the Altar entry in the left sidebar to return to the dashboard.

Dashboard cards and list rows show a **thumbnail preview** of each altar. The thumbnail is captured automatically whenever you leave edit mode — whether via the Done button, Cancel, the back-arrow breadcrumb, or switching to a different module — using a native Canvas 2D renderer and stored in the database. Altars that have not yet been saved after this feature was introduced show a live `AltarCardPreview` fallback instead. Changing the canvas aspect ratio clears the stored thumbnail so the card always shows the correct proportions. Card thumbnails are capped at a maximum height of 176 px in the regular three-column grid and 380 px in the full-width card layout, and scale proportionally without cropping.

The preview can be turned off — a chip in the filter panel — for a lighter list that shows each altar's own icon instead (or a flame, for an altar with none).
...
**Item categories** are fully dynamic, and shared with Wiki, Operations, and Tasks — assigning a category to an altar item, an article, an operation, or a task all draw from the same one list. Only **Sigils** is a true built-in, and it belongs to Operations; a fresh vault starts with eight ordinary categories (Paradigm, Ritual, Meditation, Herbs, Crystals, Candles, Deities, Tools) shared across all four, which you can rename or delete at any time. An item can also have no category at all — that is what a new one starts with. (Altar items previously drew from their own separate default set — Candle, Crystal, Herb, Deity, Symbol, Tool, Table — which existing vaults keep as ordinary categories after upgrading, merged with any same-named category another module already had.)

- Drag a category tab left or right to change its position in the tab row. Tabs animate smoothly into their new slots and the order is saved immediately to the database. Because the strip only shows the categories that hold an item, this reorders that subset within the global list.
- Adding, renaming and deleting a category happens in the Categories view (see below), not in the strip.
- Deleting a category permanently leaves its items without one; they collect under the **Uncategorized** tab, alongside items that never had a category. Moving a category to Trash instead leaves its items pointing at it, so restoring brings them back.
- Renaming a category updates all items already assigned to it — in every module that uses it, not just Altar.
- When there are more categories than can fit on screen, the tab row scrolls horizontally. The scrollbar is hidden; gradient fade overlays on the left and right edges indicate that more tabs are available in that direction. Only categories holding at least one altar item appear as tabs here; reordering only ever rearranges that visible subset, without disturbing where other, unused categories sit in the shared list.
- An item styled as a candle (🕯️ emoji) flickers regardless of which category it's filed under — that used to require the built-in "Candle" category, which no longer exists as a fixed concept.
...
**Library in the dashboard.** The Altar dashboard also shows the library, below the list of altars — a way to browse and edit items without opening an altar first. An "Altar Library" divider heading, with a count and a chevron, collapses the whole section; the collapsed state is remembered across restarts (independently of the category groups underneath it, whose collapsed state resets on restart like everywhere else). The library has its own sort (A→Z, Z→A, newest first) and Grouping toggle, shown next to the altar list's own toolbar controls in the dashboard header — independent of how the altars above it are sorted and grouped. Grouped, each category header shows a count and a "+" that opens the add-item dialog already set to that category; items whose category was deleted appear under "Uncategorized". Only categories that currently hold at least one item are shown — deliberately no empty "Other" shelf. Ungrouped, every matching item shows in one grid with no category headers. Clicking a tile opens the same edit modal as the library strip inside the altar editor. A button in the dashboard header, next to "New altar", adds a library item directly. The dashboard's search field also matches library item names alongside altar names: if only items match, the altar list shows "No results" while the library below still shows its own matches; with no altars at all, the empty state stays in place above an otherwise normal library. There is no drag-and-drop here — changing an item's category is done in the edit modal — and dragging items onto a canvas still only happens in the altar editor's own library strip.
...
**Favicon.** Each altar can have a custom favicon displayed in its tab in the tab bar and as the first row in the view-mode summary panel. To set one, open the altar in edit mode and expand the collapsible "Favicon" section at the top of the sidebar. You can pick an emoji from the shared emoji picker (the vault's own default grid — Settings → Entries, falling back to a curated built-in set — searchable by name/keyword across the full standard emoji set) or upload an image (stored directly as base64). When a favicon is set, the tab shows it instead of the default flame icon; when none is set, the flame icon is used as a fallback.
...
**Right sidebar action bar.** Entering/leaving edit mode, saving, cancelling, and deleting an open Journal entry, Wiki article, Operation, or Altar all happen from a single action bar pinned above the Properties panel in the right sidebar — not from buttons in the entry's own header, and not by double-clicking the entry itself. In view mode it shows an Edit button (plus a Fullscreen toggle for Altar); once you're editing, it shows Done, Delete (where applicable), and Cancel. An operation whose sigil is charged with the lock "whole entry" shows no actions there, since it can't be edited until unloaded.

**Cancel.** Cancelling out of edit mode reverts the title and body to what they were when edit mode was entered, even if the autosave already wrote the in-between changes to disk — typing, waiting past the autosave delay, then Cancelling discards the typing regardless. Properties-panel fields (category, tags, cover, icon) save the moment they're changed and are unaffected by Cancel; blocks, including an operation's Status block, are part of the body and reverted with it. If the entry was just created and never confirmed with Done, Cancel deletes it outright instead — permanently, with no trash and no undo — rather than leaving an empty, untitled entry behind.

**Right sidebar list header.** While a list view is showing — Home, Journal, Wiki, Operations, Tasks, the Altar dashboard, Trash, or Categories — and the right sidebar is open, the list's whole header lives there: a title row, then that view's own header actions on a full-width row of their own (a labelled "+ New …" button — on the Altar with the small "add item" icon button right beside it; Trash shows its select-all and bulk-delete controls there instead), then search, then view and sort as two rows of icon toggles, then the filter panel, permanently visible rather than needing to be toggled open. Closing the sidebar removes the header entirely rather than showing it anywhere else — deliberately, so the list gets the full height back. Tags don't have a list header, so the sidebar there just shows its usual placeholder text.

**Internal link chips.** Links inserted with `[[` are rendered as styled chips in both edit mode and view mode. There is no raw `[[Label(id)]]` text representation in edit mode. A link can point at a Journal entry, Wiki article, Operation, Task, or Altar — offered by the `[[` suggestion popup, the link picker modal (`Ctrl/Cmd+K` inside the editor, or the toolbar's link button), and by dragging a row out of the left sidebar's entry list (see above). Only Journal, Wiki, and Operations can be a link's *source* — Tasks and Altars have no editor of their own, so a link can point at one but never originate from one. Clicking a Task chip switches to the Tasks view and highlights that task's row; clicking an Altar chip opens that altar. The link picker's tabs (All/Journal/Tasks/Operations/Wiki/Altar) are icon-only, in the same order as the left rail's module icons, each showing its match count.
...
Each result shows which module it's from and, for Journal/Wiki/Operations, its entry number (e.g. `#12`). A category result shows a module too (e.g. "Category · Wiki") — since a category can be used by more than one of Wiki, Operations, Tasks, and Altar at once, this is whichever of them currently holds the most entries in it; a category nothing currently uses shows no module and can't be opened from the result. A body-text match shows a short snippet of the surrounding text with the matched word highlighted.

50 results are shown at once. If there are more, a "Show more" row at the bottom of the list reveals the next 50 — click it, or press Arrow Down on the last visible result, as many times as needed until nothing is left out. Results stay ranked as one list across every module before being cut to a page, so a module with a lot of matches can still keep other modules' weaker matches further down the list until you page down to them.

You can use the mouse or the keyboard: Up/Down moves a highlight through the list without leaving the search field, Enter opens the highlighted result, Ctrl/Cmd+Enter opens it in a new tab, and Escape clears the field and closes the dropdown in one press. Pressing Arrow Down while the last visible result is highlighted reveals the next page, the same as clicking "Show more" — once every result is showing, Arrow Down wraps back to the top like it does anywhere else in the list. Opening a Journal entry, Wiki article, Operation, or Altar takes you straight to it. Tasks and tags don't have a page of their own, so opening one of those instead takes you to the Tasks list or the Tags list, scrolls the matching row into view and marks it. The mark stays as long as that row is the one you navigated to — it doesn't fade after a moment. Opening a task also clears any search, filter or collapsed category in the Tasks view that would otherwise have hidden the row, and expands its parent tasks if it is a subtask.

There is currently no keyboard shortcut to jump to the search field. Templates and your own blocks are not included in search results — both are libraries you insert from, not entries you open (see [Templates](#templates) and the "Your own blocks" section under [Journal](#journal)).
...
A template is a pre-filled starting point for a Journal entry, Wiki article, or Operation: an optional title, a block stack — the same kind of content an entry has — and tags. The **Templates** button sits in the left rail next to Blocks; the dashboard lists templates under a collapsible "Templates" heading, with its own view (List, Cards, Cards (full width), or Timeline grouped by month) and sort (date or A–Z) like the other dashboards — each row or card shows just the icon, name, an "Unsaved" marker while a draft is open, and the entry count. Clicking one opens it as its own page — name as the title, the title new entries get, and the block stack below — shaped and saved the same way a block's own page is (see "Your own blocks" above): edits are a draft until you press Done, and a page left unfinished shows "Unsaved" in the list until you come back.

**Assignment and default** decide where a template shows up and which one a new entry starts with. Its sidebar has a collapsible "Assignment & default" section listing the combinations it currently holds — an entry type (Journal, Wiki, or Operations) and, for Wiki/Operations, a category or "All categories" — with a star next to any that are the default and a hint naming which other template a default here would replace. A button opens a dialog with the full picture as one table: Journal as its own row, then Wiki and Operations as columns against "All categories", "Uncategorized" and every category as rows; each cell switches between **not assigned**, **offered** (available when inserting, but not automatic), and **default** (what a matching new entry starts with), and a "Both" column sets a category for Wiki and Operations at once. A template with no assignment at all is offered everywhere. Edits in the dialog apply to the page's draft only once something actually changed; the page itself still saves only with Done. A collapsible, read-only "Defaults" section under the template list mirrors the same table: each cell names its default (star) and whatever else is offered there (check), links to open that template, and shows the fallback a category without its own default falls back to, or "—" where nothing applies.

**Defaults apply automatically.** A new entry starts with its combination's default: the exact category's default first, then "All categories" for that type, or nothing if neither is set. A notice above the block stack — Undo, or "Other template" — appears whenever a default just filled a brand-new entry. The built-in **Sigil calculator/drawing/charge** set a new Operation used to start with in the Sigils category is now an ordinary template, `core-sigil`, the built-in default for Operations × Sigils; it is editable and can be reassigned or deleted like any other. Changing an entry's category later re-applies the new combination's default if the entry is still empty, or if its content is still exactly what the *previous* default put there — anything you've actually written is left alone.

**Inserting one by hand.** In the editor, "Insert template" (in the blocks sidebar) opens a searchable list of templates; picking one into an entry that already has content asks whether to append the template's blocks or replace everything, and whether to bring its title and tags along. An empty entry skips the dialog and lists matching templates as buttons instead. Every entry's context menu also offers **"Save as template"**, which creates one from the entry's current content (including unsaved edits, if it's open for editing) and opens it for further work — useful for turning something you just wrote into a starting point for next time. A block a template inserts remembers where it came from, which is how the dashboard counts how many entries use a template.

**Duplicate** on a template's list row makes an independent copy — without any of the original's stars, since a template can't start out competing with itself for a default.

Deleting a template moves it to Trash (restorable, auto-purged after the vault's own retention period like everything else there); nothing it already inserted is affected, since insertion always makes a copy.

Templates replace what **Routines** used to do. Routines had lost their UI path once the right sidebar was reorganised earlier on, and templates cover the same ground — reusable content plus linked entries — and more. Every existing routine became a template automatically (same content, tags, and linked operations/articles as link-chip blocks, offered everywhere since a routine carried no assignment) the first time the vault was opened with this version; an older `.emeralddb` backup that still carries routines converts them the same way when it's imported.
...
**Categories.** The same shared category list as Wiki, Operations, and Altar items (see Altar's "Item categories" above). No category is built in on the Tasks side, and a new task starts without one, in the "Uncategorized" group; a fresh vault starts with eight ordinary, renameable/deletable categories. (A default "Allgemein"/"General" task category used to be undeletable — it is now an ordinary category, merged with any same-named category another module already had.) Categories are created, renamed and deleted in the Categories view (see below); each category header has its own "+" button that creates a new task directly in that category. Moving a category to Trash leaves its tasks pointing at it — they show in the "Uncategorized" group in the meantime — until the category is restored (bringing its tasks back with it) or deleted permanently, which then leaves the tasks without one. Categories show an emoji icon, a name, and a task count badge.
...
**Toolbar.** The list supports a search bar, four sort orders (Newest, Oldest, A–Z, Z–A), and a separate Grouping toggle (grouped by category, or ungrouped) — the two used to be one choice, "Category" being a fifth sort order; now a grouped list can also be sorted alphabetically or by date. Only the List view is available.

**Filter panel.** A "Display" chip group above the category chips holds "Show completed" — a display preference, not a filter, so it doesn't count toward the active-filter badge and survives "Clear all" (it moved here from a toolbar toggle button). Below it, filters by category (multi-select chips for the categories actually in use rather than the full global list, plus an "All" chip that clears the selection, and an "Uncategorized" chip whenever a root task's category has actually been deleted — subtasks aren't checked individually, since the "Uncategorized" group below only ever lists root tasks — staying visible for as long as it's selected) and, as its own labelled chip group next to the category one, priority (multi-select chips with a flag icon, High/Medium/Low). A "Clear all" button appears once any filter is active. When a category filter is active, only the selected categories are shown in the grouped view.

**Grouped view.** When Grouping is set to grouped, tasks are grouped under collapsible category headers. An "Uncategorized" section collects tasks without a category whenever any exist — both those that never had one and those whose category was moved to Trash. It is not a category and cannot be deleted. A category group with nothing in it is left out entirely.
...
One list of categories serves Wiki, Operations, Tasks and Altar items, and the Categories view in the left rail — next to Tags — is the one place it is managed. The four module dashboards only *assign* a category to an entry and group their lists by it; none of them can create, rename or delete one.

Each row shows the category's emoji, its name, and how many entries in each of the four modules currently use it (Tasks, Operations, Wiki, Altar — the rail's own order, with the module's icon as the column header and its name as the tooltip). Only live entries count; anything in the Trash does not.
...
- **Reorder** by dragging a row by its grip. This is the order categories appear in everywhere — group headers, filter chips, the Altar tab strip, the category picker — and it is saved immediately.
...
A category created here holds nothing yet, so it does not appear in any module until an entry points at it — assign it when creating or editing the entry, in the properties panel or the task row. Entries start with **no category** and stay under "Uncategorized" until you pick one; "Uncategorized" is the first entry of every category picker, and choosing it again removes an assignment.
...
- **Edit** and **Delete** are inline states on the same card. Edit changes the name and the icon together, in one step. Delete asks for confirmation and offers a **"Delete the files as well"** checkbox — off by default, showing the folder it would erase. It removes the vault's own files rather than the folder wholesale: the database, its journal, its images, and an empty `backup/` go, but anything else in the folder — an exported backup, a stray system file — stays, and with it the folder itself; the modal then says so, making clear the database itself is gone either way. Only a folder holding nothing but the vault is removed along with it. A folder that stayed cannot be reused for a *new* vault until it is emptied — new vaults still require an empty folder. Any vault can be deleted, including the active one and the last one remaining — deleting the active vault switches you to another one automatically, and deleting the last one returns you to vault setup.
...
- **Export.** A "What to include" chip row lets you pick which modules go into the backup: Journal, Wiki, Operations, Altars, Tasks, Tags, and Settings — there is no separate Templates chip; your templates and your own blocks travel automatically whenever Journal, Wiki, or Operations is included, the same way the backup already handled Blocks. The Settings chip (on by default) exports the vault's own settings — language, theme, fonts, sizes, trash retention, sidebar configuration, emoji defaults, image limits, tag rule — independently of the other chips, since it isn't content. Optional `dateFrom` / `dateTo` fields restrict the export to entries created in that window (all content types are date-filtered; tags, categories, blocks, templates, and settings are not). Soft-deleted rows are excluded by default; an **Include deleted** checkbox exports them too — except tags, which are always exported without their trashed rows. Embedded image files referenced from any exported entry are inlined as data-URLs. The save dialog opens in the active vault's own **`backup/` folder** — created with the vault, recreated on demand (`ensure_backup_dir`) if it went missing. The backup travels with the vault when its folder is copied, and deleting the vault's files deliberately leaves a non-empty `backup/` standing (an empty one goes with the folder); any other destination can still be picked in the dialog. Cancelling the dialog reports nothing — no success, no error. For a vault relocated outside the allowed storage roots (see [`security.md`](security.md)) `ensure_backup_dir` refuses — `write_file` could not write there anyway — and the dialog falls back to a plain filename.
- **Import.** Before importing, the modal shows a preview line summarising the counts per type (e.g. `12 J, 5 W, 3 O, 2 V, 1 A, 4 T` — "V" for templates, counting an older file's routines too, since those convert into templates on import). The same chip row is then used to choose which types to actually apply; categories can also be filtered. Three import modes are offered as selectable cards, ordered from least to most invasive — **Add Vault** (import into a brand new vault and switch to it; the preselected default, since it cannot touch existing data), **Merge** (imported entries are prefixed with a timestamp so existing IDs never collide), and **Replace** (overwrite selected tables in the current vault, with an inline warning). Add Vault asks for the new vault's name **and its folder** — the same choose-folder row the vault modal's create flow uses, defaulting to `Documents/Emerald Vaults/{name}` and updating live as the name is typed. The folder is checked before the import touches anything, under the same rules as creating a vault: it must be empty (or not exist yet), must not already hold a vault, and must be readable. Only if the default folder cannot be resolved does the vault fall back to `{appData}/vaults/{id}`. A failed import — a corrupted file, a crash partway through — leaves the vault exactly as it was before: every mode fills a hidden copy of the database and only swaps it in once the import into that copy has fully succeeded. If the file carries settings, Add Vault and Replace apply them outright; Merge instead lists the settings as checkboxes (appearance, trash, sidebar, emoji, images, tag rule) so only the groups you tick are brought in, leaving the rest of the current vault's configuration alone.
...
**Size limits.** Settings → Entries sets a per-vault maximum edge length (1024–3840px, or original) and maximum file size (1–20 MB, or unbounded) for an inserted image. Both are enforced the same way on every insert path — paste, drag-and-drop, the toolbar's image button, and a field block's image slot: the image is scaled down to the edge limit first (GIF and SVG are left alone, since scaling would lose the animation respectively the vectors), then checked against the size limit; one still too large after scaling is rejected with a notice naming the limit that applied. The same shared notice (`ImageNoticeModal`) also covers the unsupported-format case above, for the insert paths that have no notice line of their own.

## Context Menus

Right-click any entry in the left sidebar or in any list view (List, Cards, Cards in full width, Timeline layouts) to get a context menu with three core actions — most menus add **Open in new tab** as a fourth:

**Duplicate.** Creates a copy of the entry, appending a localized " (Copy)" suffix (`common.copySuffix`) to the title, and navigates to it automatically. Journal, Wiki, and Operations each go through a single store action (`duplicateEntry` / `duplicateArticle` / `duplicateOperation`) that copies every content field, blocks included. The one deliberate exception: a charged sigil comes unloaded (`withChargeUnloaded`), so the copy is editable rather than instantly locked.
...
Settings are **per vault** — stored in that vault's own `settings.json`, not shared app-wide. Switching vaults switches the whole configuration with it, and a vault folder copied to another machine keeps its own settings. The window has seven pages:
...
Settings → General → Language switches the vault's language between English, German, Spanish, and French. English is the default and fallback language; the other three locale bundles are loaded on demand the first time they are selected (`changeAppLanguage` in `src/i18n/index.ts`).

## Typography

Emerald provides two independent font controls in Settings → General:
...
**Editor text size** — a separate setting (14–22px, default 17px — the previous fixed size) for the size of text in the editor body and in field values, independent of interface size below.

## Sizing

**Interface size** (Settings → General, 90/100/110/120%, default 100%) scales the whole window as a WebView zoom rather than scaling individual elements with a root font size — so fixed-width columns and panels grow along with the text instead of the text overflowing them.

## Theming

Emerald ships with two named themes, selectable in Settings → General:
