...
**Categories.** Each article belongs to one category — the same category list Operations, Tasks, and Altar items share (see Altar's "Item categories" below for how the shared list works). Only **Other** is a true, undeletable built-in; a fresh vault additionally starts with eight ordinary, renameable/deletable categories (Paradigm, Ritual, Meditation, Herbs, Crystals, Candles, Deities, Tools). A "+ Category" button next to "New Article" in the header opens a small dialog (emoji + name) to add a custom one. The Grouping toggle groups articles by category with inline category management, independent of the sort order; each category's group can be collapsed and expanded, and its header has its own "+" button that creates a new article directly in that category. A category group with nothing in it is left out, except one just created, which keeps its empty header so there's somewhere to add its first article. Moving a category to Trash leaves its articles' category association as-is — they show in an "Uncategorized" group at the end of the list until the category is restored (bringing them back) or deleted permanently, which then moves them to **Other**. **Other** cannot be deleted. The category filter (in the toolbar's filter panel) lists only the categories actually in use by wiki articles — plus **Other** and any category just created — rather than the full global list, so categories only used by Operations, Tasks or Altar don't clutter it here; an "All" chip clears the selection, and an "Uncategorized" chip is offered whenever an article's category has actually been deleted, staying visible for as long as it's selected even once the last orphan is gone.
...
**Categories.** The same shared category list as Wiki, Tasks, and Altar items — see below. Two categories are true built-ins: **Sigils** (see Sigil Workflow) and **Other**, the fallback everything else can be renamed or deleted freely, including a category once called "Servitors" (now an ordinary category like any other). Add one via the "+ Category" button next to "New Operation" in the header. Categories have a name and an emoji; the Grouping toggle groups operations by category independent of the sort order, and their group in the list can be collapsed and expanded — each category's header also has its own "+" button that creates a new operation directly in it. A category group with nothing in it is left out, except one just created, which keeps its empty header so there's somewhere to add its first operation. Moving a category to Trash leaves its operations' category association as-is — they show in an "Uncategorized" group at the end of the list until the category is restored (bringing them back) or deleted permanently, which then moves them to **Other**. **Other** cannot be deleted. The category filter lists only the categories actually in use by operations — plus **Other** and any category just created — rather than the full global list; an "All" chip clears the selection, and an "Uncategorized" chip is offered whenever an operation's category has actually been deleted, staying visible for as long as it's selected even once the last orphan is gone.
...
**Item categories** are fully dynamic, and shared with Wiki, Operations, and Tasks — assigning a category to an altar item, an article, an operation, or a task all draw from the same one list. Only **Other** is a true built-in; a fresh vault also starts with eight ordinary categories (Paradigm, Ritual, Meditation, Herbs, Crystals, Candles, Deities, Tools) shared across all four, which you can rename or delete at any time. (Altar items previously drew from their own separate default set — Candle, Crystal, Herb, Deity, Symbol, Tool, Table — which existing vaults keep as ordinary categories after upgrading, merged with any same-named category another module already had.)
...
- Deleting a category moves its items to **Other** rather than leaving them unassigned. **Other** cannot itself be deleted, since deleting it would leave its own items with nowhere to go. An **Uncategorized** tab can still appear in the library strip as a fallback for items whose category doesn't resolve to any existing one, but it is no longer how deleting a category behaves.
...
**Right sidebar list header.** While a list view is showing — Journal, Wiki, Operations, Tasks, the Altar dashboard, or Trash — and the right sidebar is open, the list's whole header lives there instead of in a toolbar strip above the list: a title row, that view's own header actions right below it (a compact jade "+" for a new entry, and, for Wiki/Operations/Tasks, a folder-plus for "+ Category" — both with their label as a tooltip; Trash shows its select-all and bulk-delete controls there instead), then search, then view and sort as two rows of icon toggles, then the filter panel, permanently visible rather than needing to be toggled open. Closing the sidebar drops the header straight back above the list in its usual form (dropdowns for view/sort, a filter-toggle button). Home, Tags and Blocks don't have a list header, so the sidebar there just shows its usual placeholder text.
...
**Categories.** The same shared category list as Wiki, Operations, and Altar items (see Altar's "Item categories" above). Only **Other** is a true built-in and cannot be deleted; a fresh vault also starts with eight ordinary, renameable/deletable categories. (A default "Allgemein"/"General" task category used to be the undeletable one — it is now an ordinary category, merged with any same-named category another module already had.) A "+ Category" button next to "New Task" in the header opens a small dialog (emoji + name) to add one; the pencil button on each category header renames it, and each header also has its own "+" button that creates a new task directly in that category. Moving a category to Trash leaves its tasks pointing at it — they show in the "Uncategorized" group in the meantime — until the category is restored (bringing its tasks back with it) or deleted permanently, which then moves the tasks to **Other**. Categories show an emoji icon, a name, and a task count badge.
