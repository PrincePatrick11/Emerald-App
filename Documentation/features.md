...
4. In read mode, charge the sigil — optionally after a timer — and confirm. A charge without a reveal date hides what it covers until you unload it, rather than showing an unset date. Until the reveal date (or the unload), the calculators and drawings it covers are hidden (also on list cards and in search) and left out of exports; from the reveal date on they show again, still locked. "Unload sigil" (with confirmation) lifts the lock. A duplicated sigil comes unloaded, and a block a charge is currently hiding or locking can't be duplicated at all — the copy would carry the same charge without actually being covered by it.
