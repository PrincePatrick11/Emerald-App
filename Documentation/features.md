...
**Right sidebar list header.** While a list view is showing — Home, Journal, Wiki, Operations, Tasks, the Altar dashboard, Trash, or Categories — and the right sidebar is open, the list's whole header lives there instead of in a toolbar strip above the list: a title row, then that view's own header actions on a full-width row of their own (a labelled "+ New …" button — on the Altar with the small "add item" icon button right beside it; Trash shows its select-all and bulk-delete controls there instead), then search, then view and sort as two rows of icon toggles, then the filter panel, permanently visible rather than needing to be toggled open. Closing the sidebar drops the header straight back above the list in its usual form (dropdowns for view/sort, a filter-toggle button). Tags doesn't have a list header, so the sidebar there just shows its usual placeholder text.
