...
**Your own blocks.** The **Blocks** view (rail, above Tags) builds blocks out of fields, no code needed — a "Ritual log" with date, choice and checklist, for example. Pick an emoji and a name, add fields by kind, label and reorder them (drag by the grip), set the display rules — hide empty fields in read mode, show the block's title, fully read-only — and save; until then "Discard" returns to the saved version. The block then appears under "Add block" in every entry, after the built-in ones. What lands in an entry is a **copy**: changing the block later leaves existing entries as they are. While editing such an entry, the block's frame shows "Newer version", and its "…" menu offers "Update block" (Cancel reverts it like any other change); the Blocks view shows how many entries use a block and how many of them carry an older version, and "Update all" brings them up to date at once — except an entry that is open for editing right then. Updating keeps every value; new fields arrive empty, and a field you removed keeps its data in the entries but stays hidden — the Blocks view lists removed fields and can bring them back. Deleting a block moves it to the trash and leaves every copy in the entries untouched; to remove it from all entries as well, tick "Also remove it from N entries" and confirm a second time — that part can only be undone from a backup. With no block selected, the view gives an overview of the built-in blocks.
