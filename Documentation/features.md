...
**List views.** The journal list supports three layouts (List, Cards, Timeline) and five sort orders (newest first, oldest first, A→Z, Z→A, and Category — which for the journal groups by **moon phase**). Sorting by Category renders one collapsible group per phase — chevron, moon emoji, phase name, entry count — in fixed lunar-cycle order, including phases with no entries; entries without a phase get their own "Without moon phase" group. Each group can be folded independently, matching the category groups in Wiki, Operations and Tasks. The filter panel lists all eight phases as chips regardless of whether they're in use, plus an "All" chip that clears the selection and a "Without moon phase" chip, always available; deselecting every phase hides all entries, and selecting a single empty phase shows its header instead of "No results". A leading **"Only with entries"** chip, separated from the phase chips by a divider, hides every phase group that currently has nothing in it — it counts toward the active-filter badge and resets with "Clear all". Search filters by title *or* tag name. **Timeline view locks out A→Z, Z→A, and Category sorting** — they're shown greyed out with a tooltip explaining why, since the timeline already orders entries by date and would ignore them anyway; the same restriction applies wherever else Timeline is offered as a layout (Wiki, Operations, the Altar dashboard).
