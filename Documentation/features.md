...
**Right sidebar list header.** While a list view is showing — Journal, Wiki, Operations, Tasks, the Altar dashboard, or Trash — and the right sidebar is open, the list's whole header lives there instead of in a toolbar strip above the list: a title row, that view's own header actions right below it (a compact jade "+" for a new entry, and, for Wiki/Operations/Tasks, a folder-plus for "+ Category" — both with their label as a tooltip; Trash shows its select-all and bulk-delete controls there instead), then search, then view and sort as two rows of icon toggles, then the filter panel, permanently visible rather than needing to be toggled open. Closing the sidebar drops the header straight back above the list in its usual form (dropdowns for view/sort, a filter-toggle button). Home and Tags don't have a list header, so the sidebar there just shows its usual placeholder text.
