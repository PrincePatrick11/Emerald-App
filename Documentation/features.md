...
A template is a pre-filled starting point for a Journal entry, Wiki article, or Operation: an optional title, a description, a block stack — the same kind of content an entry has — and tags. The **Templates** button sits in the left rail next to Blocks; the dashboard lists every template with its assignments, its entry count, and a star if it is a default somewhere, and clicking one opens it as its own page — name as the title, description, the title new entries get, and the block stack below — shaped and saved the same way a block's own page is (see "Your own blocks" above): edits are a draft until you press Done, and a page left unfinished shows "Unsaved" in the list until you come back.

**Assignments** decide where a template shows up. In its sidebar, assign it to a combination — an entry type (Journal, Wiki, or Operations) and, for Wiki/Operations, a category or "All categories" (Journal has no categories, so it only ever gets "All categories") — and optionally star it as the **default** for that combination; starring one takes the star from whoever held it before. A template with no assignment at all is offered everywhere. A collapsible "Defaults" section under the template list shows the whole picture at once — Journal once, Wiki and Operations with "All categories", "Uncategorized" and every category — with a dropdown to change each combination's default directly; a category with no default of its own shows which one it falls back to.
