...
**Cancel.** Cancelling out of edit mode reverts the title and body to what they were when edit mode was entered, even if the autosave already wrote the in-between changes to disk — typing, waiting past the autosave delay, then Cancelling discards the typing regardless. Properties-panel fields (category, tags, cover, icon, status, end date, version) save the moment they're changed and are unaffected by Cancel. If the entry was just created and never confirmed with Done, Cancel deletes it outright instead — permanently, with no trash and no undo — rather than leaving an empty, untitled entry behind.
