...
{
  "version": 2,
  "vaults": [{ "id": "default", "name": "Emerald", "path": "…/vaults/default", "createdAt": "…" }],
...
- **`resolve_allowed_roots()`** deliberately leaves the registered directories out — see [`security.md`](security.md). Vault storage resolves by id and never consults those roots, so a vault outside `~` works in full; only writing a *document* into such a folder is out of reach.
...
| `delete_vault_files(vault_id)` | Remove the vault's own files by name, then the two directories with non-recursive `remove_dir`. A folder holding anything foreign survives and reports `VAULT_DIR_NOT_EMPTY`. |
...
The window's title bar is drawn by the app, not the OS — a slim 40px bar holding the Emerald logo, the application menu, back/forward navigation, a search field, and the window buttons (`src/components/layout/titlebar/`). It sits above the three-column shell in `AppShell`.
