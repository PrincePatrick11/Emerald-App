...
Tauri derives `app_data_dir()` / `app_config_dir()` from the app's `identifier`, and the identifier changed with 0.2.x (`com.emerald.magical-journal` → `com.emerald.app`, see [`build.md`](build.md#product-name-vs-identifier)). Every installation up to v0.1.3.6 wrote its `vaults.json` under the old identifier's directory; without extra work the new identifier would point at an empty one, and a long-time user would open Emerald to a blank journal.

`adopt_previous_identifier_dirs` (`src-tauri/src/vault.rs`) copies that old directory's contents across, once, on first start:

- **Copies, never moves.** The previous installation is only read, never touched, so it keeps working as a fallback if the copy fails or the user wants to go back.
- **Only its own artifacts** — `vaults.json`, `images/`, `vaults/`, and anything named `emerald*` (the flat legacy `.db` files, their `-journal` sidecars, the `.pre-vNN.bak` migration backups) — never the whole directory. On Linux, `app_data_dir` is also where Tauri creates the WebView's profile (`dirs::data_local_dir()` and `data_dir()` are the same call there), so the old directory can hold that install's cookies and `localStorage` too, which have no business in the new profile.
- **All-or-nothing per run.** Each entry is written next to its final name and only made visible by a same-directory `rename`, so nothing ever sits half-copied under its real name; if any entry fails, everything this run placed is rolled back, and the next start tries again from a clean slate.
- **Gated on "does the target already hold anything of ours," not on emptiness** — Linux's WebView profile is already living there before this code runs at all, so an empty-directory check would never fire. Only the presence of the named artifacts above counts.
- **Rewrites the copied `vaults.json`'s absolute paths** from the old root to the new one, for any vault that had been migrated under the pre-0.2.1 layout (`{appDataDir}/vaults/{id}`, see [Migrating an older installation](#migrating-an-older-installation) below) — otherwise the copy would keep pointing at the old install's folder while sitting next to an increasingly stale twin. See [`security.md`](security.md#vault-directories-as-a-trust-boundary) for the path-safety rule this rewrite follows.

**It runs before there is a window, as a Tauri plugin's own `setup`** (registered in `lib.rs` as `tauri::plugin::Builder::new("emerald-adopt")`), not the app's own `setup` block that [Loading Screen and Boot Order](#loading-screen-and-boot-order) below picks up from. Tauri's own startup builds the configured windows *before* calling the app's `setup` hook — and creating that first window is exactly what creates the WebView profile under `LocalData/{identifier}` on Windows and Linux. A plugin `setup` runs inside `Builder::build()`, ahead of the event loop and every window, so the adoption always sees its target directory before the WebView has had a chance to occupy it — and the copy itself blocks no window thread. It runs over both `app_data_dir` and `app_config_dir`, since those are two different directories on Linux (see the same split under [Migrating an older installation](#migrating-an-older-installation) just below).
...
An earlier stage than anything below runs before this: `adopt_previous_identifier_dirs` (see [Adopting a previous identifier's data](#adopting-a-previous-identifiers-data) under Vault Layout) executes inside `Builder::build()`, before Tauri creates the window this section's markup ever paints into.
