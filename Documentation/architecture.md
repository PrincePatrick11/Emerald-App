...
- **`src/components/altar/AltarCanvas.tsx`** — canvas scene rendering, placement transforms, drag/drop interactions, lock handling, and grid overlay drawing. Also exports `captureCurrentAltar(): Promise<string | null>` — a standalone async function that reads altar state from `useAltarStore.getState()` synchronously and renders a thumbnail on an off-screen canvas without touching the DOM. It is safe to call after `AltarCanvas` has unmounted (e.g. from a `useEffect` cleanup). The `captureRef` prop mechanism that previously threaded a capture callback through the component tree was removed in favour of this module-level export.
