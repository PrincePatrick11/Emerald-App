...
- **Temporary:** the Export menu (Markdown / PDF / Emerald) is disabled while an Operations entry is open. Export for that module isn't wired up correctly yet, so the menu items are greyed out to avoid exposing a broken action, rather than being left to fail silently. Journal, Wiki, and Altar reading-view export are unaffected. This will be re-enabled once Operations export is implemented. (`src/components/layout/AppShell.tsx`)
