...
- Altar view mode sidebar: **Save Image button** — a "Bild speichern…" / "Save Image…" button in the reading summary sidebar renders the active altar at its full native resolution as JPEG (quality 0.97) and saves it to a user-chosen path via the native OS save dialog; default filename is `AltarTitle_YYYY-MM-DD.jpg`; the button shows in-progress, success, and error feedback states; i18n in de/en/fr/es
...
- Altar sidebar: **overlay color toggle** — the "Overlay Options" section (collapsible, between Background and Grid Options) now includes two buttons to switch the overlay between Dark (black gradient, `rgba(10,10,15,…)`) and Light (white gradient, `rgba(255,255,255,…)`); the choice is stored per altar in `background_overlay_color` and is reflected in both the live canvas and the thumbnail renderer
...
- `resolveResolutionPixels(res)` exported from `src/lib/altarConstants.ts` — resolves any stored resolution string (ratio format like `'4:3'` or pixel format like `'1920x1080'`) to concrete pixel dimensions by mapping ratio strings through `ALTAR_RESOLUTION_MAP.lg`; replaces direct `parseResolution` calls on `altar.resolution` across dashboard components and the thumbnail renderer
...
- Altar library strip: hover a category tab to reveal a pencil edit button; a "+ Category" button is always visible outside the scrollable tab row so it is reachable regardless of scroll position
