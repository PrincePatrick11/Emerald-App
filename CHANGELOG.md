...
- Altar export: **format selection** — `exportCurrentAltarImage()` accepts an optional `format` parameter (`'jpeg' | 'png' | 'webp'`, default `'jpeg'`); JPEG uses quality 0.97, WebP uses 0.92, PNG is lossless; the format picker in the sidebar passes the selected format and adjusts the OS save dialog filter and suggested filename extension accordingly
